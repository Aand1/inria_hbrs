...
namespace gazebo
{
  namespace physics
  {
    ...
    /// children.
    class GZ_PHYSICS_VISIBLE DummyPerception : public State
    {
      /// \brief Default constructor
      public: DummyPerception();

      /// \brief Constructor.
      ///
      /// Generate a WorldState from an instance of a World.
      /// \param[in] _world Pointer to a world
      public: explicit DummyPerception(const WorldPtr _world);

      /// \brief Constructor
      ///
      /// Build a WorldState from SDF data
      /// \param[in] _sdf SDF data to load a world state from.
      public: explicit DummyPerception(const sdf::ElementPtr _sdf);

      /// \brief Destructor.
      public: virtual ~DummyPerception();
